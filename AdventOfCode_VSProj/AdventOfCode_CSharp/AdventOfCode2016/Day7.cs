...
namespace AdventOfCode_CSharp.AdventOfCode2016
{
    ...
    class Day7
    {
        public static void RunDay7()
        {
            ...
            //int totalMatch = ResolvePartOneTotalMatch(lines);
            int totalMatch = ResolvePartTwoTotalMatch(lines);
        }

        static int ResolvePartOneTotalMatch(List<string> lines)
        {
            int totalSupportedIPs = 0;

            foreach (var line in lines)
            {
                string[] lineSections = line.Split(new char[] { '[', ']' });
                string abbaPattern = "(\\w)(\\w)\\2\\1";

                bool anyABBAMatchInsideSquareBrackets = false;
                bool anyABBAMatchOutsideSquareBrackets = false;

                for (int section = 1; section <= lineSections.Length; ++section)
                {
                    bool sequenceInSquareBrackets = (section % 2 == 0);

                    System.Text.RegularExpressions.Match match = System.Text.RegularExpressions.Regex.Match(lineSections[section - 1], abbaPattern);

                    if (match.Success && !match.Value[0].Equals(match.Value[1]))
                    {
                        if (sequenceInSquareBrackets)
                        {
                            anyABBAMatchInsideSquareBrackets = true;
                            break;
                        }
                        else
                        {
                            anyABBAMatchOutsideSquareBrackets = true;
                        }
                    }
                }

                if (anyABBAMatchOutsideSquareBrackets == true && anyABBAMatchInsideSquareBrackets == false)
                {
                    totalSupportedIPs += 1;
                    System.Diagnostics.Debug.WriteLine("Find ABBA match: " + line);
                }
            }
            System.Diagnostics.Debug.WriteLine("Total match: " + totalSupportedIPs);

            return totalSupportedIPs;
        }

        static int ResolvePartTwoTotalMatch(List<string> lines)
        {
            int totalSupportedIPs = 0;
            ...
            {
                string[] lineSections = line.Split(new char[] { '[', ']' });                
                bool ipSupportSSL = false;
                List<string> matchesInsideSquareBrackets = new List<string>();
                List<string> matchesOutsideSquareBrackets = new List<string>();
                ...
                for (int section = 1; section <= lineSections.Length; ++section)
                {
                    ...
                    if (sequenceInSquareBrackets)
                    {
                        matchesInsideSquareBrackets.AddRange(GetABAPatternSubStrings(lineSections[section - 1]));
                    }
                    else
                    {
                        matchesOutsideSquareBrackets.AddRange(GetABAPatternSubStrings(lineSections[section - 1]));
                    }
                }

                foreach (var abaMatchOut in matchesOutsideSquareBrackets)
                {
                    foreach (var abaMatchIn in matchesInsideSquareBrackets)
                    {
                        // Check any ABA outside has related BAB inside.
                        string str = abaMatchIn[1].ToString() + abaMatchIn[0].ToString() + abaMatchIn[1].ToString();
                        if (abaMatchOut.Equals(str))
                        {
                            ipSupportSSL = true;
                            break;
                        }
                    }
                    if (ipSupportSSL == true)
                    {
                        break;
                    }
                }
                ...
                {
                    totalSupportedIPs += 1;
                    System.Diagnostics.Debug.WriteLine("Find SSL support: " + line);
                }
            }
            System.Diagnostics.Debug.WriteLine("Total match: " + totalSupportedIPs);
        ...
        static List<string> GetABAPatternSubStrings(string str)
        {
            ...
            for (int i = 0; i < str.Length - 2; ++i)
            {
                ...
                if (subStr[0] == subStr[2] && !subStr[0].Equals(subStr[1]) && !resultList.Contains(subStr))
                {
        ...
        // Part 1: Y N N Y
        readonly static string input_1_1 = @"abba[mnop]qrst
                                            abcd[bddb]xyyx
                                            aaaa[qwer]tyui
                                            ioxxoj[asdfgh]zxcvbn
                                            rnqfzoisbqxbdlkgfh[lwlybvcsiupwnsyiljz]kmbgyaptjcsvwcltrdx[ntrpwgkrfeljpye]jxjdlgtntpljabbaxaojufe";

        // Part 2:
        readonly static string input_1_2 = @"abacacaaa[bab]xyz
                                            xyx[xyx]xyx
                                            aaa[kek]eke
                                            zazbz[bzb]cdb";
        ...
        // Part 1: 105, Part 2:
        #region LARGE_INPUT
        readonly static string input_2 = @"dnwtsgywerfamfv[gwrhdujbiowtcirq]bjbhmuxdcasenlctwgh
