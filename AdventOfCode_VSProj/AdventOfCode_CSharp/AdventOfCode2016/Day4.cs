...
namespace AdventOfCode_CSharp.AdventOfCode2016
{
    class Day4
    {
        public static void RunDay4()
        {
            ...
            {
                // last 2 entry are "sector ID", and "checksum"
                string[] splitted = Helper.InputHandler.GetSplitSubstringsWithSeparators(line[0], new string[] { "-", "[", "]" });
                string letters = "";
                string encryptedRoomName = "";

                for (int i = 0; i < splitted.Length - 2; ++i)
                {
                    letters += splitted[i];

                    encryptedRoomName += splitted[i];
                ...
                Tuple<string, int> roomNameAndId = RoomInfo.GetRealRoomNameWithShiftCipher(encryptedRoomName, int.Parse(splitted[splitted.Length - 2]));
                System.Diagnostics.Debug.WriteLine("RoomNameAndId: " + roomNameAndId.Item1 + " | " + roomNameAndId.Item2);

                Tuple<bool, int> isRealRoomAndSectorId = RoomInfo.IsARealRoom(letters, int.Parse(splitted[splitted.Length - 2]), splitted[splitted.Length - 1]);

                if (isRealRoomAndSectorId.Item1)
                {
                    sumOfRealRoomSectorId += isRealRoomAndSectorId.Item2;
                }
            }
            System.Diagnostics.Debug.WriteLine("SumOfRealRoomSectorId: " + sumOfRealRoomSectorId);
        }

        class RoomInfo
        {
            public static Tuple<bool, int> IsARealRoom(string letters, int sectorId, string checksum)
            {
                ...
                foreach (var letter in letters)
                {
                    ...
                    {
                        int count = letterCounter[letter];
                ...
                List<Tuple<char, int>> letterCounterList = new List<Tuple<char, int>>();
                foreach (KeyValuePair<char, int> kv in letterCounter)
                {
                    letterCounterList.Add(Tuple.Create(kv.Key, kv.Value));
                }
                List<Tuple<char, int>> sortedLetterCounterList = letterCounterList.OrderByDescending(t => t.Item2).ThenBy(t => t.Item1).ToList();

                for (int i = 0; i < checksum.Length; ++i)
                {
                    if (!checksum[i].Equals(sortedLetterCounterList[i].Item1))
                    {
                        return Tuple.Create(false, sectorId);
                    }
                }
                return Tuple.Create(true, sectorId);
            }

            public static Tuple<string, int> GetRealRoomNameWithShiftCipher(string encryptedRoomName, int sectorId)
            {
                string decryptedRoomName = "";
                int shift = sectorId % 26;

                foreach (var c in encryptedRoomName)
                {
                    if (c == '-')
                    {
                        decryptedRoomName += ' ';
                    ...
                    {
                        char charAfterShift;
                        if (((c + shift) > 'z')) // wrap needed
                        {
                            charAfterShift = (char)((c + shift) - ('z' - 'a' + 1));
                        }
                        else
                        {
                            charAfterShift = (char)(c + shift);
                        }
                        decryptedRoomName += charAfterShift;
                    }
                }
                return Tuple.Create(decryptedRoomName, sectorId);
            }
        }
        

        // Part 1: 1514,Part 2: 
        static readonly string input_1 = @" qzmt-zixmtkozy-ivhz-343[abcde]
        ...
        // v v v x
        #region INPUT
        // Part 1: 278221,Part 2:
        static readonly string input_2 = @" aczupnetwp-dnlgpyrpc-sfye-dstaatyr-561[patyc]
